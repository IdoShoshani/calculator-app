...
# ── IAM Role for SSM (no SSH needed from CI/CD) ──────────────────────────────
...
# ── SSH Key Pair (generated by Terraform, saved locally) ─────────────────────
resource "tls_private_key" "ssh" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "aws_key_pair" "calculator" {
  key_name   = "calculator-app-key"
  public_key = tls_private_key.ssh.public_key_openssh
}

# Writes the private key to calculator-key.pem next to this file (git-ignored)
resource "local_sensitive_file" "private_key" {
  content         = tls_private_key.ssh.private_key_pem
  filename        = "${path.module}/calculator-key.pem"
...
resource "aws_instance" "calculator" {
  ...
  key_name               = aws_key_pair.calculator.key_name
  subnet_id              = aws_subnet.public.id
